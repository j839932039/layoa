...
layui.define(['laytpl', 'laypage', 'layer', 'form', 'util'], function(exports){
  ...
  ,table = {
    config: {
      ...
      ,restful:true // 自定义修改 --思途 2020-02-08 为了解决 分页时的url符合restful的格式
    } // 全局配置项
    ,cache: {} // 数据缓存
  ...
  Class.prototype.pullData = function(curr){
    ...
    if(options.url){ // Ajax请求
      ...
      // 思途修改 2020-02-08 为了适应 restful格式的请求
      options.sendurl = options.url;
      var params = {};
      // 参数
      var data;
      // 判断是否是restful格式的请求
      if(options.restful){
    	  options.sendurl =  options.url+"/"+curr+"/"+options.limit;
    	// 参数
         data = options.where;
      }else{
      ...
      if(options.contentType && options.contentType.indexOf("application/json") == 0){ // 提交
																						// json
																						// 格式
  ...
  Class.prototype.events = function(){
    ...
    // 复选框选择
    that.elem.on('click', 'input[name="layTableCheckbox"]+', function(){ // 替代元素的
																			// click
																			// 事件
      ...
      layui.each(item1, function(i2, item2){
        ...
        if(item2.colGroup){
          ...
          layui.each(cols[i1 + 1], function(i22, item22){
            // 如果子列已经被标注为{PARENT_COL_INDEX}，或者子列累计 colspan 数等于父列定义的
			// colspan，则跳出当前子列循环
